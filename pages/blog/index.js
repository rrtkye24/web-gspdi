...
function MetaHeader() {
  const seoConfig = {
    ...
    title: "Artikel - GSPDI Filadelfia Indonesia",
    description:
  ...
  const fetchPosts = async () => {
    ...
    try {
      const recentPosts = await client.fetch(
        `*[_type == 'post' && "Artikel" in categories[]->title ] | order(_publishedAt desc)[${postCount}..${postCount + perPage
        }]{
              slug,
              title,
              excerpt,
...
export async function getStaticProps() {
  const recentPosts = await client.fetch(
    `*[_type == 'post' && "Artikel" in categories[]->title ] | order(_publishedAt desc)[0..8]{
          slug,
          title,
          excerpt,
          description,
